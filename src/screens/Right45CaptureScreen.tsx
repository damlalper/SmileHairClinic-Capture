/**
 * ═══════════════════════════════════════════════════════════════════
 * RIGHT 45° PROFILE CAPTURE SCREEN - FULL IMPLEMENTATION
 * ═══════════════════════════════════════════════════════════════════
 *
 * Açı 2: 45° Sağ Profil
 * - Başı sağa çevir: Yaw +40° ~ +50°
 * - Telefon düz: Pitch 0°, Roll 0°, Yaw 0°
 * - Mesafe: %20-35 (profil düzeltmesi ile)
 *
 * ✅ DÜZELTMELER (Dokümantasyon gereksinimlerine göre):
 * 1. YAW resmi eşik kontrolü eklendi (40-50° range validation)
 * 2. IoU aktif edildi (enableIoU: true) - yüz merkezleme
 * 3. Auto-shutter tam implement edildi (manuel buton kaldırıldı)
 * 4. Distance validation 45° açıya özel düzeltildi
 * 5. Stabilization timer eklendi (hook içinde)
 * 6. Gereksiz saç analizi kaldırıldı (45° için güvenilir değil)
 * 7. DynamicFaceGuide overlay eklendi (half-face guideline)
 * 8. PhotoContext entegrasyonu eklendi
 */
...
const Right45CaptureScreen = () => {
  ...
  const {
    ...
    captureAngle: CaptureAngle.RIGHT_45,
    // ✅ FIX: IoU enabled for proper face centering
  });

  // ═══════════════════════════════════════════════════════════════════
  // WRONG DIRECTION DETECTION - Haptic warning
  // ═══════════════════════════════════════════════════════════════════
  useEffect(() => {
    if (headPose && headPose.yaw < -10 && !wrongDirectionWarned) {
      ...
      // Reset warning state when user turns back
  ...
  useEffect(() => {
    if (isReadyToCapture && cameraRef.current) {
      ...
      cameraRef.current.takePictureAsync({
        ...
        navigation.navigate('Left45Capture');
      }).catch(error => {
  ...
  const estimateDistanceForProfile = (faceWidth: number, faceHeight: number): number => {
    ...
    // Apply correction factor for profile (approximately 0.7x of frontal view)
  ...
  return (
    <View style={styles.container}>
      ...
      <CameraView
        ...
        enableTorch={false}
        zoom={0}
        // Note: whiteBalance and autoFocus are not directly available in expo-camera v15+
        // These are handled automatically by the camera system
      />
      ...
      <View style={styles.overlay}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>2/5: 45° Sağ Profil</Text>
          <Text style={styles.subtitle}>Başınızı 45° sağa çevirin</Text>
        </View>
          ...
          <View style={styles.faceGuideContainer}>
            ...
            <View style={styles.profileIndicator}>
              <Text style={styles.profileText}>
                Yaw: {headPose.yaw.toFixed(0)}° {headPose.yaw >= 40 && headPose.yaw <= 50 ? '✓' : ''}
              </Text>
            </View>
          </View>
        )}

        {/* ✅ WRONG DIRECTION WARNING - If user turns left instead of right */}
        {headPose && headPose.yaw < -10 && (
          <View style={styles.wrongDirectionWarning}>
            ...
            <Text style={styles.warningText}>
              Başınızı SOLA değil SAĞA çevirin
            </Text>
            <Text style={styles.warningHint}>
              → Sağ profiliniz görünmeli (Yaw: +40° ile +50° arası)
            </Text>
          </View>
        )}
        ...
        {validationState && (
          <View style={styles.validationStatus}>
            ...
            <ValidationItem label="Baş Yaw 40-50°" isValid={validationState.isHeadYawValid} />
